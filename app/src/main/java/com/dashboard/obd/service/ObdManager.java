...
public class ObdManager implements OnObdStateListener, OnObdDataListener, OnObdExtraDataListener,
    ...
    public void run(String obd_Address,boolean autoScan, boolean allowBluetoothControl) {
        synchronized (this) {


            BluetoothDevice device = BluetoothAdapter.getDefaultAdapter().getRemoteDevice(obd_Address);//getBluetoothDevice(obd_Address);
            if (device == null) {
                logger.warn("# invalid obd device: address=" + obd_Address);
                return;
            }
            ...
            if (!mRunning) {
                ...
                config.carId = "capa_001";//vehicle.getVehicleId();
                config.fuelType = FuelType.GASOLINE;// vehicle.getFuelType();
                config.engineDisplacement =2400;// vehicle.getModel().getDisplacement();
                config.protocol = "6";//"Auto";//vehicle.getObdProtocol();
                config.connectionMethod = 0;//vehicle.getObdConnectionMethod();
...
//                Logger.getLogger(TAG).debug(mIsISG + "#" + vehicle.toString());
    ...
    public void run(String obd_Address,boolean autoScan) {
        run(obd_Address,autoScan, true);
    }
    ...
    private BluetoothDevice getBluetoothDevice(String address) {
        ...
        try {
            ...
            if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
                logger.debug("run(): " + address + " is not bonded!");
            }
            return device;
        } catch (Exception e) {
            logger.debug("run(): invalid bluetooth address=" + address);
            return null;
        }
    }
    ...
    public void resetIfNeeded(boolean isNew) {
        ...
        if (isNew) {
            ...
            mProcessor.resetTrip(new ObdPostProcessor.OnTripResetCallback() {
